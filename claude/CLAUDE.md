...
- **`/qwen`, `/deepseek`, `/deepseek-r`** — implemented here as real slash commands at
  `~/.claude/commands/{qwen,deepseek,deepseek-r}.md`. They shell out via the `Bash` tool
  to the `qwen` CLI / DeepSeek API directly (not the OMP `task` tool). Use them exactly
