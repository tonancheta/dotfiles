...
# Own filename, for self-referential messages ("re-run this script") that
# must say the right thing in bootstrap-new.sh too -- that file is a
# variant of this one (see its own header comment) and both must tell the
...
check_symlinks() {
    ...
    if ln -sf "$DOTFILES_DIR/bootstrap.sh" "$probe" 2>/dev/null; then
        ...
        echo "    linking them — re-run $SCRIPT_NAME after editing anything in this repo."
...
if [ -d "$DOTFILES_DIR/claude/scripts" ]; then
    ...
    if [ -f "$DOTFILES_DIR/claude/scripts/package.json" ] && command -v npm &> /dev/null; then
        ...
        if ! (cd "$DOTFILES_DIR/claude/scripts" && npm install); then
            echo "⚠️  npm install failed for claude/scripts — its helper scripts may not work until you resolve this and re-run $SCRIPT_NAME."
        fi
    fi
fi
...
# 9c. Register bootstrap-autorun.sh (repo root) as a login/new-shell hook,
# so whichever bootstrap variant last ran (this file or bootstrap-new.sh)
# stays in effect on subsequent logins without a manual re-run. The rc
# line itself is identical no matter which variant wrote it -- it just
# sources the shared autorun script, which decides what to actually run
# by reading the state file step 9d writes below. Same self-healing
# rewrite pattern as steps 9/9b.
AUTORUN_LINE="[ -f \"$DOTFILES_DIR/bootstrap-autorun.sh\" ] && source \"$DOTFILES_DIR/bootstrap-autorun.sh\""
for rc in "$HOME/.bashrc" "$HOME/.zshrc"; do
    if [ -f "$rc" ] && ! grep -qF "$AUTORUN_LINE" "$rc"; then
        sed -i.bak -e '/^# Bootstrap autorun (dotfiles)$/d' -e '/bootstrap-autorun\.sh/d' "$rc" && rm -f "$rc.bak"
        printf '\n# Bootstrap autorun (dotfiles)\n%s\n' "$AUTORUN_LINE" >> "$rc"
        echo "✅ Registered bootstrap-autorun in $rc"
    fi
done

# 9d. Record THIS script as the active bootstrap variant for
# bootstrap-autorun.sh above. Running bootstrap-new.sh instead of this
# file (or vice versa) overwrites this one-line state file with its own
# path -- that's the entire mechanism behind "subsequent logins run
# whichever variant you last ran manually"; the rc line above never has
# to change.
mkdir -p "$HOME/.omp/agent"
